...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_me = make_city("me", lat, lon)
    dis1 = distance(city_me, city_a)
    dis2 = distance(city_me, city_b)
    if dis1 < dis2:
...
def riffle(deck):
    ...
    while i < length:
        if i % 2 == 0:
            ...
            half_num = len(arr) // 2
            all += [arr[half_num]]
            arr = arr[:half_num] + arr[(half_num + 1):]
        i += 1
    return all
...
def add_trees(t1, t2):
    ...
    if is_leaf(t2):
        ...
        if num1 > num2:
            zip_t = list(zip(b1, b2))
            return tree(label(t1) + label(t2), [add_trees(x, y) for x, y in zip_t] + b1[num2:])
        elif num2 > num1:
            zip_t = list(zip(b1, b2))
            return tree(label(t1) + label(t2), [add_trees(x, y) for x, y in zip_t] + b2[num1:])
        else:
            zip_t = list(zip(b1, b2))
            return tree(label(t1) + label(t2), [add_trees(x, y) for x, y in zip_t])
