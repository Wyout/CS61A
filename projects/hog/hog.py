...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        ...
        GCD_now = 1
        GCD = 1
        num = min(player_score, opponent_score)
        cal = 0
        while cal <= num and GCD <= 10:
            if player_score % (GCD_now+1) == 0 and opponent_score % (GCD_now+1) == 0:
                GCD = GCD_now + 1
            GCD_now += 1
            cal += 1
    if GCD < 10:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            player0_numroll = strategy0(score0, score1)
            this_turn_roll = take_turn(player0_numroll, score1, dice)
            score0 += this_turn_roll
            if not extra_turn(score0, score1):
                who = other(who)
        else:
            player1_numroll = strategy1(score1, score0)
            this_turn_roll = take_turn(player1_numroll, score0, dice)
            score1 += this_turn_roll
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    #这里看了别人的代码，他用了指针（数组）。在这一节其实没有讲
    def say(*score):
        assert len(score) == 2
        new_high = score[who] - last_score
        if new_high > running_high:
            print(new_high, 'point(s)! The most yet for Player', who)
            return announce_highest(who, score[who], new_high)
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def say(num = 1, dice = original_function):
        all = 0
        cal = 0
        while cal < trials_count:
            all += roll_dice(num, dice)
            cal += 1
        res = all / cal
        return res
    return say
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # args = [averaged_dice(i, dice) for i in range(1, 11)]
    # return args.index(max(args)) + 1

    # 自己的代码
    my_av = make_averaged(roll_dice, trials_count)
    cal = 1
    res = 0
    while cal < 11:
        new_res = my_av(cal, dice)
        if new_res > res:
            true_res = cal
        cal += 1
        res = new_res
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if extra_turn(free_bacon(opponent_score)+score, opponent_score) or \
        free_bacon(opponent_score) >= cutoff else num_rolls  # Replace this statement
    # END PROBLEM 11
