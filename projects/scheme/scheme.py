...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        the_args = args
        while the_args != nil:
            python_args.append(the_args.first)
            the_args = the_args.rest
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        func = do_lambda_form(Pair(formal, body), env)
...
def make_let_frame(bindings, env):
    ...
    the_bindings = bindings
    while the_bindings is not nil:
        a = the_bindings.first
        validate_form(a, 2, 2)

        name = a.first
        value = scheme_eval(a.rest.first, env)

        # 记住这种写法
        names = Pair(name, names)
        values = Pair(value, values)

        the_bindings = the_bindings.rest
